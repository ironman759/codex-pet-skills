...
STUDIO_BRIEF_PATH = "references/studio-brief.json"
...
def load_studio_brief(path: Path | None) -> dict[str, object]:
    if path is None:
        return {}
    if not path.is_file():
        raise SystemExit(f"studio brief not found: {path}")
    try:
        brief = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as error:
        raise SystemExit(f"invalid studio brief: {error}") from error
    if not isinstance(brief, dict):
        raise SystemExit("invalid studio brief: root must be a JSON object")

    identity_lock = brief.get("identity_lock", {})
    if not isinstance(identity_lock, dict):
        raise SystemExit("invalid studio brief: identity_lock must be an object")
    for key in ("must_preserve", "must_not_add"):
        values = identity_lock.get(key, [])
        if not isinstance(values, list) or not all(
            isinstance(value, str) and value.strip() for value in values
        ):
            raise SystemExit(f"invalid studio brief: identity_lock.{key} must be a string array")

    action_contract = brief.get("action_contract", {})
    if not isinstance(action_contract, dict):
        raise SystemExit("invalid studio brief: action_contract must be an object")
    valid_states = {state for state, _row, _frames, _purpose in ROWS}
    unknown_states = sorted(set(action_contract) - valid_states)
    if unknown_states:
        raise SystemExit(
            "invalid studio brief: unknown action_contract states: "
            + ", ".join(unknown_states)
        )
    if not all(
        isinstance(value, str) and value.strip() for value in action_contract.values()
    ):
        raise SystemExit("invalid studio brief: action_contract values must be strings")

    look_mechanics = brief.get("look_mechanics", "")
    if not isinstance(look_mechanics, str):
        raise SystemExit("invalid studio brief: look_mechanics must be a string")
    return brief


def studio_contract_block(
    args: argparse.Namespace,
    *,
    state: str = "",
    include_look: bool = False,
) -> str:
    brief = getattr(args, "studio_brief", {})
    if not brief:
        return ""

    lines: list[str] = []
    identity_lock = brief.get("identity_lock", {})
    must_preserve = identity_lock.get("must_preserve", [])
    must_not_add = identity_lock.get("must_not_add", [])
    if must_preserve:
        lines.append("Identity hard lock — preserve: " + "; ".join(must_preserve))
    if must_not_add:
        lines.append("Identity hard lock — do not add: " + "; ".join(must_not_add))

    action = brief.get("action_contract", {}).get(state, "") if state else ""
    if action:
        lines.append(f"Approved `{state}` action semantics: {compact(action)}")
    if include_look and brief.get("look_mechanics", "").strip():
        lines.append("Approved look mechanics: " + compact(brief["look_mechanics"]))
    if not lines:
        return ""
    return "\nStudio-approved contract:\n" + "\n".join(f"- {line}" for line in lines) + "\n"
...
Style: {style_contract}
{studio_contract_block(args, state=state)}
...
{studio_contract_block(args, include_look=True)}
...
def make_jobs(
    ...
    approved_base_source: Path | None = None,
    ...
    base_job: dict[str, object] = {
        "id": "base",
        "kind": "base-pet",
        "status": "complete" if approved_base_source else "pending",
    ...
    if approved_base_source:
        base_job.update(
            {
                "source_path": str(approved_base_source),
                "completed_at": datetime.now(timezone.utc).isoformat(),
                "approved_source": True,
            }
        )
    jobs: list[dict[str, object]] = [base_job]
    for state, _row, frames, _purpose in ROWS:
        depends_on = ["base"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--approved-base",
        default="",
        help="User-approved main pet art to use directly as the canonical identity source.",
    )
    parser.add_argument(
        "--studio-brief",
        default="",
        help="Approved Jarvis Pet Studio JSON contract to copy into the run and inject into prompts.",
    )
    ...
    raw_approved_base_path = (
        Path(args.approved_base).expanduser().resolve()
        if args.approved_base.strip()
        else None
    )
    raw_studio_brief_path = (
        Path(args.studio_brief).expanduser().resolve()
        if args.studio_brief.strip()
        else None
    )
    ...
    identity_source_paths = [*raw_reference_paths]
    if raw_approved_base_path is not None:
        identity_source_paths.append(raw_approved_base_path)
    args.studio_brief = load_studio_brief(raw_studio_brief_path)
    args.display_name = infer_name(args, identity_source_paths)
    args.pet_name = (args.pet_name or args.display_name).strip()
    args.description = infer_description(args, identity_source_paths)
    args.pet_notes = infer_pet_notes(args, identity_source_paths)
    ...
    approved_base_path = ""
    if raw_approved_base_path is not None:
        if not raw_approved_base_path.is_file():
            raise SystemExit(f"approved base not found: {raw_approved_base_path}")
        decoded_base = run_dir / "decoded" / "base.png"
        canonical_base = run_dir / CANONICAL_BASE_PATH
        try:
            with Image.open(raw_approved_base_path) as source_image:
                approved_image = source_image.convert("RGBA")
                approved_image.save(decoded_base)
                approved_image.save(canonical_base)
        except (OSError, ValueError) as error:
            raise SystemExit(f"invalid approved base: {error}") from error
        copied_ref_paths.append(canonical_base)
        approved_base_path = rel(canonical_base, run_dir)

    studio_brief_path = ""
    if raw_studio_brief_path is not None:
        copied_studio_brief = run_dir / STUDIO_BRIEF_PATH
        copied_studio_brief.write_text(
            json.dumps(args.studio_brief, indent=2) + "\n", encoding="utf-8"
        )
        studio_brief_path = rel(copied_studio_brief, run_dir)
        look_mechanics = compact(str(args.studio_brief.get("look_mechanics", "")))
        if look_mechanics:
            write_text(run_dir / "qa" / "look-mechanics.md", look_mechanics + "\n")
    ...
    if approved_base_path:
        request["approved_base_path"] = approved_base_path
        request["approved_base_source"] = str(raw_approved_base_path)
    if studio_brief_path:
        request["studio_brief_path"] = studio_brief_path
    ...
    job_list = make_jobs(run_dir, copied_refs, raw_approved_base_path)
    jobs = {
        "schema_version": 1,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "run_dir": str(run_dir),
    ...
    completed_ids = {job["id"] for job in job_list if job["status"] == "complete"}
    ready_jobs = [
        job["id"]
        for job in job_list
        if job["status"] != "complete"
        and all(dependency in completed_ids for dependency in job["depends_on"])
    ]
    print(
        json.dumps(
            {
