...
### Approved Studio Handoff

When Jarvis Pet Studio has already approved the main art, pass it through `--approved-base` and pass the single approved design contract through `--studio-brief`. The prepared run copies the approved art directly into `decoded/base.png` and `references/canonical-base.png`, marks the base job complete, and injects the brief's identity locks, state-specific action semantics, and look mechanics into the existing prompts. Do not regenerate the base or reinterpret an approved lock.

The studio brief is JSON. It may include `source_character`, `source_interpretation`, `design_axes`, `identity_lock.must_preserve`, `identity_lock.must_not_add`, `action_contract`, `look_mechanics`, `animation_energy`, `naming`, and `approvals`. Keep the brief as the single design contract rather than splitting identity, action, and look decisions across competing files.
...
- If a higher-priority user or workspace policy requires original generated images to be retained, preserve them and use that policy's archive command instead of the deletion step above.
...
"$PYTHON" "$SKILL_DIR/scripts/prepare_pet_run.py" \
  ...
  --approved-base /absolute/path/to/approved-main-art.png \
  --studio-brief /absolute/path/to/studio-brief.json \
...
For an approved Jarvis Pet Studio handoff, pass both `--approved-base` and `--studio-brief`. The approved base becomes the canonical identity without another base generation; the brief is copied into `references/studio-brief.json`, and its look mechanics are written to `qa/look-mechanics.md` when present.
